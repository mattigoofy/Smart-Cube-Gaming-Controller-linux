...
@dataclass
class CharacterCounterSettings:
    # Count "A" as "shift+a"
    allow_capital_letters: bool = False
    # Insert 'ghost' modifier keys like ctrl, wind, ... because those are (probably) not in the dataset
    apply_fake_modifier_keys: bool = True


class CharacterCounter:
    def __init__(self) -> None:
        self._settings: CharacterCounterSettings = CharacterCounterSettings()
        self._counter: Counter[str] | None = None

    @property
    def counter(self):
        if self._counter is None:
            raise ValueError("No counter available. Please run `count` first.")
        return self._counter
    
    def settings(self, new_settings: CharacterCounterSettings):
        self._settings = new_settings
        return self
    
    def count(self, limit: int | None = None) -> None:
        raise NotImplementedError
    
    def count_post_processing(self):
        if not self._settings.apply_fake_modifier_keys:
            return
        max_count = max(self.counter.values())
        ghost_count = int(max_count * 0.1)
        for _ in range(ghost_count):
            self.counter.update(["ctrl"])
            self.counter.update(["alt"])
            self.counter.update(["fn"])
            self.counter.update(["win"])
    
    def update_counter(self, new: str):
        if self._settings.allow_capital_letters:
        ...
        for char in new:
            if char.isupper():
                # Split into shift+char
                self.counter.update(["shift"])
                self.counter.update([char.lower()])
            else:
    ...
    def save_frequencies(self, output_path: str):
        ...
        data = {
            "total_chars": sum(self.counter.values()),
            "unique_chars": len(self.counter),
    ...
    def print_top(self, n: int = 30):
        print(f"\nTop {n} characters:")
        print(f"{'Rank':>5}  {'Char':>6}  {'Codepoint':>10}  {'Count':>12}  {'Name'}")
        print("-" * 65)
        import unicodedata
        for rank, (ch, count) in enumerate(self.counter.most_common(n), 1):
            try:
...
class WikipediaDumpCharacterCounter(CharacterCounter):
    ...
    def _iter_article_texts(self, xml_path: str):
        """
            ...
            if event == "start":
                ...
                if tag == f"{ns}redirect":
                    ...
                    if not is_redirect and current_text:
                        try:
                            yield self._plain_text_from_wikitext(current_text)
                        except Exception:
    ...
    def count(self, limit: int | None = None) -> None:
        ...
        for text in self._iter_article_texts(self._xml_path):
            self.update_counter(text)
            pages_processed += 1

            if pages_processed % 1000 == 0:
                print(f"  {pages_processed:,} articles processed, "
                    f"{len(self._counter):,} unique chars so far...", flush=True)

            if limit and pages_processed >= limit:
                print(f"Reached --limit {limit}, stopping early.")
                break

        self.count_post_processing()
        print(f"\nDone. {pages_processed:,} articles, {len(self._counter):,} unique characters.")

# TODO make GenericCounter class instead
